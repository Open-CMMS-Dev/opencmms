...
export function WorkOrderView({ workOrderId, onNavigate }: WorkOrderViewProps) {
  ...
  const loadWorkOrder = async () => {
    try {
      setLoading(true)
      const data = await workOrdersApi.getById(workOrderId)
      setWorkOrder(data)
    } catch (error) {
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-2">
          <Button 
            variant="outline"
            onClick={() => onNavigate(`edit/${workOrder.id}`)}
            className="flex items-center gap-2"
          >
